...
6. 优化前行情 worker 把 819 个日价格和 481 个历史快照逐行写入远端数据库，一次同步实测 `877 s`。批量写入与 Alpaca 主供应商切换后，同一组 39 个真实标的全量同步实测 `17.399–19.260 s`；详细对比见 [MARKET_DATA_PROVIDER_BENCHMARK.md](MARKET_DATA_PROVIDER_BENCHMARK.md)。
...
当时 Twelve Data 配置为每个 symbol 一次请求，两个请求之间等待 8 秒：
...
当前实现已将默认供应商切换为 Alpaca：日线按 50 个 symbol 批量请求并处理分页，网络任务并发上限为 4，客户端限速为 180 RPM；仅在 Alpaca 失败或缺少 symbol 时，才按单 symbol、8 秒间隔调用 Twelve Data。上述 877 秒数据仍保留为优化前性能基线。
